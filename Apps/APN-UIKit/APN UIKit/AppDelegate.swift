...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        // TESTING-ONLY (geofence-testing branch): mark process launch and whether iOS
        // relaunched us for a location event — the "did we wake at all" cold-wake signal.
        // Runs after initializeCioAndInAppListeners so GeofenceTestNotifier.onDebug is live.
        GeofenceTestHooks.onDebug?("AppDelegate: didFinishLaunching (locationLaunch=\(launchOptions?[.location] != nil))")
...
enum GeofenceTestNotifier {
    ...
    static func install() {
        ...
        // Every geofence debug line (OS register/deregister, enter/exit callbacks, cold-wake
        // bootstrap, routing, cooldown suppress/pass) → an on-device notification. Fired from
        // arbitrary SDK threads, so hop to main.
        GeofenceTestHooks.onDebug = { message in
            DispatchQueue.main.async { postDebugNotification(message) }
        }
    }

    private static func postDebugNotification(_ message: String) {
        let content = UNMutableNotificationContent()
        content.title = "Geofence"
        content.body = message
        content.sound = .default
        let request = UNNotificationRequest(
            identifier: "gf-debug-\(Date().timeIntervalSince1970)",
            content: content,
            trigger: nil
        )
