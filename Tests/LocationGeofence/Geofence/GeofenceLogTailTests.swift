...
struct GeofenceLogTailTests {
    ...
    /// Shared with the scenario replay harness — see `Stub/GeofenceTailCapture.swift`. One parser
    /// for one contract: a second reading here would drift from what replay asserts.
    ...
    private var invocations: [Invocation] {
        ...
        return [
            ...
            Invocation(name: "callbackReceived", ev: "os.callback.received", requiredKeys: ["id", "t", "buf", "fixsrc", "acc", "age", "sim", "evage"]) { $0.geofenceCallbackReceived(identifier: "notl_core", transition: .enter, fix: location, source: .managerCache, eventDate: Date(timeIntervalSinceNow: -3), buffered: false, now: Date()) },
    ...
    func everyRecord_expectMachineKeyAndReplayClassification() {
        withDiagnostics(true) {
            ...
            for invocation in invocations {
                ...
                // Which one, not merely that it is one of the three. The weaker check is how the
                // Android side drifted to 24 outputs against this module's seven without a single
                // test noticing — every one of them would have become a hard expectation in a
                // generated scenario.
    ...
    ///
    /// `out` is the entire assertion surface. Anything absent here is `obs` — readable in a
    /// capture, never graded. Pinned per `ev`, not per call site, because the key is what a parser
    /// dispatches on: two `module.reset` sites that classified themselves differently would make
    /// the key mean whichever one happened to fire.
    ///
    /// Kept in step with Android's `GeofenceLogTailTest.declaredIo` deliberately — the keys are
    /// shared, so a divergence here is a divergence in what the two platforms' scenarios assert.
    ...
    /// Named separately from `declaredIo` so the size of the assertion surface is visible.
    ///
    /// Three, since 2026-09-11. An output is a decision that crosses back out of the SDK — a region
    ...
    /// is a transition that is or is not accepted, which is already here. Asserting them pinned every
    /// drive to one implementation's internals, and two iOS drives failed on nothing else.
    ...
    ///
    /// The number is the point. A scenario asserts exactly this set, so a ninth output silently
    /// widens every recorded drive's expectations and a missing one silently narrows them.
    @Test
    ...
    /// The complete set of `ev=` keys the module is allowed to emit. Data, not logic — kept
    /// at type level so the check below reads as one assertion.
