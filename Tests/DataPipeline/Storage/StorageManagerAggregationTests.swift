...
class StorageManagerAggregationTests: XCTestCase {
    ...
    func test_setAndGetAggregationConfig_roundTrip() throws {
        try storage.setAggregationConfig(payload: #"{"filters":[]}"#, fetchedAt: "2026-05-28T00:00:00Z")

        let result = try XCTUnwrap(storage.getAggregationConfig())
        XCTAssertEqual(result.payload, #"{"filters":[]}"#)
        XCTAssertEqual(result.fetchedAt, "2026-05-28T00:00:00Z")
    }

    func test_setAggregationConfig_upsert_overwritesPreviousValue() throws {
        try storage.setAggregationConfig(payload: #"{"filters":[]}"#, fetchedAt: "2026-05-01T00:00:00Z")
        try storage.setAggregationConfig(payload: #"{"rateLimits":[]}"#, fetchedAt: "2026-05-28T00:00:00Z")

        let result = try XCTUnwrap(storage.getAggregationConfig())
        XCTAssertEqual(result.payload, #"{"rateLimits":[]}"#)
        XCTAssertEqual(result.fetchedAt, "2026-05-28T00:00:00Z")
    }
