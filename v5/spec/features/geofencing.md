# Geofencing — Feature Specification
...
public struct GeofenceConfig: Sendable {
    ...
    ///
    /// Temporary — will be derived from SdkConfig.region once backend paths
    /// are finalised. When set, the coordinator runs the cursor-based diff
    /// sync against the CIO-hosted endpoint on each startup. When nil (the
    /// default), the SDK seeds from the bundled example file once and makes
...
coordinate at `GeofenceConfig.quadKeyZoom`. This derived value is written to
the local DB; it is **not** part of the JSON schema.
...
`CustomerIO_Geofencing` target.

### `geofences`

Stores the imported geofence dataset. Rebuilt atomically on each successful
full-reload sync.
...
`geofence_state` has no foreign-key constraint to `geofences`. Full-reload
syncs delete `geofence_state` rows explicitly before replacing `geofences`.
Incremental syncs clean up state rows per deletion via `applyGeofenceDiff`.
...
`GeofenceSyncClient` implements the cursor-based diff sync. On each startup,
if `syncURL` is configured, the coordinator calls `sync(url:token:)` once.

| Endpoint | When called |
|---|---|
| `GET <syncURL>?token=<cursor>` | On each SDK startup when `syncURL` is set. Token omitted on first sync. |

For the full wire format, request/response field definitions, and server
implementation guidance, see
[`spec/interfaces/geofence-sync-api.md`](../interfaces/geofence-sync-api.md).
...
- As the default dataset before the first server sync completes (when `syncURL`
  is configured and the local DB is empty).
