...
/**
 Class to easily create a mocked version of the `PushNotificationsUtil` class. 
 This class is equipped with functions and properties ready for you to mock! 

 Note: This file is automatically generated. This means the mocks should always be up-to-date and has a consistent API.
 See the SDK documentation to learn the basics behind using the mock classes in the SDK. 
 */
internal class PushNotificationsUtilMock: PushNotificationsUtil, Mock {
    /// If *any* interactions done on mock. `true` if any method or property getter/setter called. 
    internal var mockCalled: Bool = false // 
    ...
    public func resetMock() {
        showPromptForPushNotificationPermissionCallsCount = 0
        showPromptForPushNotificationPermissionReceivedArguments = nil 
        showPromptForPushNotificationPermissionReceivedInvocations = []

        self.mockCalled = false // do last as resetting properties above can make this true
    }

    // MARK: - showPromptForPushNotificationPermission

    /// Number of times the function was called.  
    internal private(set) var showPromptForPushNotificationPermissionCallsCount = 0
    /// `true` if the function was ever called. 
    internal var showPromptForPushNotificationPermissionCalled: Bool {
        return showPromptForPushNotificationPermissionCallsCount > 0
    }    
    /// The arguments from the *last* time the function was called. 
    internal private(set) var showPromptForPushNotificationPermissionReceivedArguments: (options: [String: Any], onComplete: (Result<Bool, Error>) -> Void)?
    /// Arguments from *all* of the times that the function was called. 
    internal private(set) var showPromptForPushNotificationPermissionReceivedInvocations: [(options: [String: Any], onComplete: (Result<Bool, Error>) -> Void)] = []
    /** 
     Set closure to get called when function gets called. Great way to test logic or return a value for the function. 
     */
    internal var showPromptForPushNotificationPermissionClosure: (([String: Any], @escaping (Result<Bool, Error>) -> Void) -> Void)?

    /// Mocked function for `showPromptForPushNotificationPermission(options: [String: Any], onComplete: @escaping (Result<Bool, Error>) -> Void)`. Your opportunity to return a mocked value and check result of mock in test code.
    internal func showPromptForPushNotificationPermission(options: [String: Any], onComplete: @escaping (Result<Bool, Error>) -> Void) {
        self.mockCalled = true
        showPromptForPushNotificationPermissionCallsCount += 1
        showPromptForPushNotificationPermissionReceivedArguments = (options: options, onComplete: onComplete)
        showPromptForPushNotificationPermissionReceivedInvocations.append((options: options, onComplete: onComplete))
        showPromptForPushNotificationPermissionClosure?(options, onComplete)
    }
