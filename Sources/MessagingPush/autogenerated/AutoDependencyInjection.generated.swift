...
public class DIMessagingPush {    
    ...
    */
    public func inject<T>(_ dep: DependencyMessagingPush) -> T {                            
        switch dep {
            case .moduleHookProvider: return self.moduleHookProvider as! T 
            case .queueRunnerHook: return self.queueRunnerHook as! T 
        }
    }
    ...
    */
    ...
    internal var moduleHookProvider: ModuleHookProvider {  
        if let overridenDep = self.overrides[.moduleHookProvider] {
    ...
    public var queueRunnerHook: QueueRunnerHook {  
        if let overridenDep = self.overrides[.queueRunnerHook] {
            return overridenDep as! QueueRunnerHook
        }
        return self.newQueueRunnerHook
    }
    private var newQueueRunnerHook: QueueRunnerHook {    
        return MessagingPushQueueRunner(siteId: self.siteId, diTracking: self.dITracking)
    }
}
