...
class MessagingPushImplementation: MessagingPushInstance {
    ...
    func trackMetricFromNSE(
        ...
        let result = await DIGraphShared.shared.richPushDeliveryTracker.trackMetric(token: deviceToken, event: event, deliveryId: deliveryID, timestamp: nil)
        switch result {
        case .success:
            pushLogger.logPushMetricTracked(deliveryId: deliveryID, event: event.rawValue)
        case .failure(let error):
            pushLogger.logPushMetricTrackingFailed(deliveryId: deliveryID, event: event.rawValue, error: error)
        }
