...
public class EventBusMock: EventBus, Mock {
    ...
    /// The arguments from the *last* time the function was called.
    public private(set) var postReceivedArguments: (event: AnyEventRepresentable, queue: DispatchQueue?)?
    /// Arguments from *all* of the times that the function was called.
    public private(set) var postReceivedInvocations: [(event: AnyEventRepresentable, queue: DispatchQueue?)] = []
     ...
     */
    public var postClosure: ((AnyEventRepresentable, DispatchQueue?) -> Bool)?

    /// Mocked function for `post(_ event: AnyEventRepresentable, on queue: DispatchQueue?)`. Your opportunity to return a mocked value and check result of mock in test code.
    @discardableResult
    public func post(_ event: AnyEventRepresentable, on queue: DispatchQueue?) -> Bool {
        mockCalled = true
        postCallsCount += 1
        postReceivedArguments = (event: event, queue: queue)
        postReceivedInvocations.append((event: event, queue: queue))
        return postClosure.map { $0(event, queue) } ?? postReturnValue
    }
