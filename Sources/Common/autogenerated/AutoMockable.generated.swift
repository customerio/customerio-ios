...
/**
 ...
 See the SDK documentation to learn the basics behind using the mock classes in the SDK.
 */
class EventStorageMock: EventStorage, Mock {
    /// If *any* interactions done on mock. `true` if any method or property getter/setter called.
    var mockCalled: Bool = false //
    ...
    public func resetMock() {
        ...
        mockCalled = false // do last as resetting properties above can make this true
        loadAllEventsCallsCount = 0
        loadAllEventsReceivedArguments = nil
        loadAllEventsReceivedInvocations = []

        mockCalled = false // do last as resetting properties above can make this true
        clearEventCallsCount = 0
        clearEventReceivedArguments = nil
        clearEventReceivedInvocations = []

        mockCalled = false // do last as resetting properties above can make this true
        clearAllEventsCallsCount = 0

        mockCalled = false // do last as resetting properties above can make this true
    }

    // MARK: - store<E: Codable>

    var storeThrowableError: Error?
    /// Number of times the function was called.
    private(set) var storeCallsCount = 0
    /// `true` if the function was ever called.
    ...
    /// The arguments from the *last* time the function was called.
    private(set) var storeReceivedArguments: (event: AnyEncodable, key: String)?
    /// Arguments from *all* of the times that the function was called.
    private(set) var storeReceivedInvocations: [(event: AnyEncodable, key: String)] = []
    /**
     Set closure to get called when function gets called. Great way to test logic or return a value for the function.
     */
    var storeClosure: ((AnyEncodable, String) throws -> Void)?

    /// Mocked function for `store<E: Codable>(event: E, forKey key: String)`. Your opportunity to return a mocked value and check result of mock in test code.
    func store<E: Codable>(event: E, forKey key: String) throws {
        ...
        mockCalled = true
        storeCallsCount += 1
        storeReceivedArguments = (event: AnyEncodable(event), key: key)
        storeReceivedInvocations.append((event: AnyEncodable(event), key: key))
        try storeClosure?(AnyEncodable(event), key)
    }

    // MARK: - loadAllEvents

    var loadAllEventsThrowableError: Error?
    /// Number of times the function was called.
    private(set) var loadAllEventsCallsCount = 0
    /// `true` if the function was ever called.
    var loadAllEventsCalled: Bool {
        loadAllEventsCallsCount > 0
    }

    /// The arguments from the *last* time the function was called.
    private(set) var loadAllEventsReceivedArguments: (type: any EventRepresentable.Type, key: String)?
    /// Arguments from *all* of the times that the function was called.
    private(set) var loadAllEventsReceivedInvocations: [(type: any EventRepresentable.Type, key: String)] = []
    /// Value to return from the mocked function.
    var loadAllEventsReturnValue: [any EventRepresentable]!
    /**
     Set closure to get called when function gets called. Great way to test logic or return a value for the function.
     The closure has first priority to return a value for the mocked function. If the closure returns `nil`,
     then the mock will attempt to return the value for `loadAllEventsReturnValue`
     */
    var loadAllEventsClosure: ((any EventRepresentable.Type, String) throws -> [any EventRepresentable])?

    /// Mocked function for `loadAllEvents(ofType type: any EventRepresentable.Type, withKey key: String)`. Your opportunity to return a mocked value and check result of mock in test code.
    func loadAllEvents(ofType type: any EventRepresentable.Type, withKey key: String) throws -> [any EventRepresentable] {
        if let error = loadAllEventsThrowableError {
            throw error
        }
        mockCalled = true
        loadAllEventsCallsCount += 1
        loadAllEventsReceivedArguments = (type: type, key: key)
        loadAllEventsReceivedInvocations.append((type: type, key: key))
        return try loadAllEventsClosure.map { try $0(type, key) } ?? loadAllEventsReturnValue
    }

    // MARK: - clearEvent

    var clearEventThrowableError: Error?
    /// Number of times the function was called.
    private(set) var clearEventCallsCount = 0
    /// `true` if the function was ever called.
    var clearEventCalled: Bool {
        clearEventCallsCount > 0
    }

    /// The arguments from the *last* time the function was called.
    private(set) var clearEventReceivedArguments: String?
    /// Arguments from *all* of the times that the function was called.
    private(set) var clearEventReceivedInvocations: [String] = []
    /**
     Set closure to get called when function gets called. Great way to test logic or return a value for the function.
     */
    var clearEventClosure: ((String) throws -> Void)?

    /// Mocked function for `clearEvent(forKey key: String)`. Your opportunity to return a mocked value and check result of mock in test code.
    func clearEvent(forKey key: String) throws {
        if let error = clearEventThrowableError {
            throw error
        }
        mockCalled = true
        clearEventCallsCount += 1
        clearEventReceivedArguments = key
        clearEventReceivedInvocations.append(key)
        try clearEventClosure?(key)
    }

    // MARK: - clearAllEvents

    var clearAllEventsThrowableError: Error?
    /// Number of times the function was called.
    private(set) var clearAllEventsCallsCount = 0
    /// `true` if the function was ever called.
    var clearAllEventsCalled: Bool {
        clearAllEventsCallsCount > 0
    }
    ...
    /**
     Set closure to get called when function gets called. Great way to test logic or return a value for the function.
     */
    var clearAllEventsClosure: (() throws -> Void)?

    /// Mocked function for `clearAllEvents()`. Your opportunity to return a mocked value and check result of mock in test code.
    func clearAllEvents() throws {
        if let error = clearAllEventsThrowableError {
            throw error
        }
        mockCalled = true
        clearAllEventsCallsCount += 1
        try clearAllEventsClosure?()
    }
}
