...
public class LoggerMock: Logger, Mock {
    ...
    /// The arguments from the *last* time the function was called.
    @Atomic public private(set) var setLogDispatcherReceivedArguments: LogDispatcher??
    /// Arguments from *all* of the times that the function was called.
    @Atomic public private(set) var setLogDispatcherReceivedInvocations: [LogDispatcher?] = []
    /**
     Set closure to get called when function gets called. Great way to test logic or return a value for the function.
     */
    public var setLogDispatcherClosure: ((LogDispatcher?) -> Void)?

    /// Mocked function for `setLogDispatcher(_ dispatcher: LogDispatcher?)`. Your opportunity to return a mocked value and check result of mock in test code.
