...
extension DIGraph {
    ...
    public var globalKeyValueStorage: GlobalKeyValueStorage {  
        return getOverriddenInstance() ??
            self.newGlobalKeyValueStorage
    }
    private var newGlobalKeyValueStorage: GlobalKeyValueStorage {    
        return GlobalKeyValueStorage(siteId: self.siteId, deviceMetricsGrabber: self.deviceMetricsGrabber)
    }
    // JsonAdapter
