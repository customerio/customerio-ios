...
extension DIGraph {
    ...
    private var newQueue: Queue {    
        return CioQueue(siteId: self.siteId, storage: self.queueStorage, runRequest: self.queueRunRequest, jsonAdapter: self.jsonAdapter, logger: self.logger, sdkConfig: self.sdkConfig, queueTimer: self.singleScheduleTimer, dateUtil: self.dateUtil, threadUtil: self.threadUtil)
    }
    // QueueQueryRunner
    ...
    public var threadUtil: ThreadUtil {  
        ...
        return self.sharedThreadUtil
    }
    public var sharedThreadUtil: ThreadUtil {
        // Use a DispatchQueue to make singleton thread safe. You must create unique dispatchqueues instead of using 1 shared one or you will get a crash when trying 
        // to call DispatchQueue.sync{} while already inside another DispatchQueue.sync{} call. 
        return DispatchQueue(label: "DIGraph_ThreadUtil_singleton_access").sync {
            if let overridenDep = self.overrides[String(describing: ThreadUtil.self)] {
                return overridenDep as! ThreadUtil
            }
