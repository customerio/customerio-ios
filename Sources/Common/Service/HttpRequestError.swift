...
extension HttpRequestError: CustomStringConvertible, LocalizedError {
    ...
    public var description: String {
        let messageWhenApiResponseWasNull = "(no helpful message from the API)"
        ...
        case .unsuccessfulStatusCode(let code, let apiMessage):
            return "Response received, but status code = \(String(code)). \(apiMessage ?? messageWhenApiResponseWasNull)"
        case .noOrBadNetwork: return "No Internet connection or bad network connection."
        case .badRequest400(let apiMessage):
            return "HTTP request responded with 400 - \(apiMessage ?? messageWhenApiResponseWasNull)"
