...
extension DIGraph {
    ...
    internal var inAppProvider: InAppProvider {  
        if let overridenDep = self.overrides[String(describing: InAppProvider.self)] {
            return overridenDep as! InAppProvider
        }
        return self.newInAppProvider
    }
    ...
    private var newModuleHookProvider: ModuleHookProvider {    
        return MessagingInAppModuleHookProvider(siteId: self.siteId)
    }
}
