...
extension DIGraphShared {
    ...
    internal var anonymousMessageManager: AnonymousMessageManager {
        return getOverriddenInstance() ??
            self.sharedAnonymousMessageManager
    }
    internal var sharedAnonymousMessageManager: AnonymousMessageManager {
        // Use a DispatchQueue to make singleton thread safe. You must create unique dispatchqueues instead of using 1 shared one or you will get a crash when trying
        // to call DispatchQueue.sync{} while already inside another DispatchQueue.sync{} call.
        return DispatchQueue(label: "DIGraphShared_AnonymousMessageManager_singleton_access").sync {
    ...
    private func _get_inAppMessageManager() -> InAppMessageManager {
        return InAppMessageStoreManager(logger: self.logger, threadUtil: self.threadUtil, logManager: self.logManager, gistDelegate: self.gistDelegate, anonymousMessageManager: self.anonymousMessageManager)
    }
    // LogManager
    ...
    private func _get_queueManager() -> QueueManager {
        return QueueManager(keyValueStore: self.sharedKeyValueStorage, gistQueueNetwork: self.gistQueueNetwork, inAppMessageManager: self.inAppMessageManager, anonymousMessageManager: self.anonymousMessageManager, logger: self.logger)
    }
}
