...
/**
 Class to easily create a mocked version of the `ViewAnimationRunner` class.
 This class is equipped with functions and properties ready for you to mock!

 Note: This file is automatically generated. This means the mocks should always be up-to-date and has a consistent API.
 See the SDK documentation to learn the basics behind using the mock classes in the SDK.
 */
class ViewAnimationRunnerMock: ViewAnimationRunner, Mock {
    /// If *any* interactions done on mock. `true` if any method or property getter/setter called.
    var mockCalled: Bool = false //
    ...
    public func resetMock() {
        animateCallsCount = 0
        animateReceivedArguments = nil
        animateReceivedInvocations = []
    ...
    /// Number of times the function was called.
    @Atomic private(set) var animateCallsCount = 0
    /// `true` if the function was ever called.
    var animateCalled: Bool {
        animateCallsCount > 0
    }

    /// The arguments from the *last* time the function was called.
    @Atomic private(set) var animateReceivedArguments: (duration: TimeInterval, delay: TimeInterval, options: UIView.AnimationOptions, animations: () -> Void, completion: ((Bool) -> Void)?)?
    /// Arguments from *all* of the times that the function was called.
    @Atomic private(set) var animateReceivedInvocations: [(duration: TimeInterval, delay: TimeInterval, options: UIView.AnimationOptions, animations: () -> Void, completion: ((Bool) -> Void)?)] = []
    /**
     Set closure to get called when function gets called. Great way to test logic or return a value for the function.
     */
    var animateClosure: ((TimeInterval, TimeInterval, UIView.AnimationOptions, @escaping () -> Void, ((Bool) -> Void)?) -> Void)?

    /// Mocked function for `animate(withDuration duration: TimeInterval, delay: TimeInterval, options: UIView.AnimationOptions, animations: @escaping () -> Void, completion: ((Bool) -> Void)?)`. Your opportunity to return a mocked value and check result of mock in test code.
    func animate(withDuration duration: TimeInterval, delay: TimeInterval, options: UIView.AnimationOptions, animations: @escaping () -> Void, completion: ((Bool) -> Void)?) {
        mockCalled = true
        animateCallsCount += 1
        animateReceivedArguments = (duration: duration, delay: delay, options: options, animations: animations, completion: completion)
        animateReceivedInvocations.append((duration: duration, delay: delay, options: options, animations: animations, completion: completion))
        animateClosure?(duration, delay, options, animations, completion)
    }
}
