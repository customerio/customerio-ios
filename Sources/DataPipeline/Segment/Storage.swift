...
class Storage {
    ...
    private var flushPolicies: [FlushPolicy] {
        DataPipeline.moduleConfig.flushPolicies
    }
...
extension Storage {
    private func currentFile(_ key: Storage.Constants) -> URL {
        ...
        currentFile = index
        return eventsFile(index: currentFile)
    }
    ...
    func eventFiles(includeUnfinished: Bool) -> [URL] {
        ...
        if includeUnfinished == false {
            files = allFiles?.filter { file -> Bool in
                file.pathExtension == Storage.tempExtension
            }
        }
