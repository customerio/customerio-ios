...
extension Logger {
    ...
    /// `heldFix` records what became of a fix the caller supplied: reused, too old to stand in for
    /// a request, or replaced by a newer one this resolver already held. The difference between a
    /// re-evaluation that decides and one that records `no_usable_fix` for every polygon.
