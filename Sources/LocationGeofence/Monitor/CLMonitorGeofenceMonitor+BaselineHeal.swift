    ...
    func enqueueBaselineHeal(candidates: [String]) {
        ...
        enqueueMonitorOperation { [weak self] _ in
            ...
            for (identifier, condition) in expectedConditions.sorted(by: { $0.key < $1.key }) {
                ...
                // `.suppressedNewerBaseline` — the OS path judges an event by its own identity
                // (`osEventDate`) and passes no evidence timestamp — so without this the token
                // exists but can never print.
