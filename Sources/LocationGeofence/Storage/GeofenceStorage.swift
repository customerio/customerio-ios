...
actor GeofenceStorage {
    ...
    /// Clock for stored timestamps. `lastStateChangedAt` is evidence — `recordMonitorEvent` weighs
    /// it against a fix's timestamp — so it must come from the clock the fixes come from.
