...
internal class HttpRetryPolicyMock: HttpRetryPolicy, TrackingMock {
    ...
    /**
     ...
     When the getter of the property called, the value set here will be returned. Your chance to mock the property.
     */
    internal var underlyingNextSleepTimeMilliseconds: Milliseconds?
    /// `true` if the getter or setter of property is called at least once.
    internal var nextSleepTimeMillisecondsCalled: Bool {
        nextSleepTimeMillisecondsGetCalled || nextSleepTimeMillisecondsSetCalled
    }

    /// `true` if the getter called on the property at least once.
    internal var nextSleepTimeMillisecondsGetCalled: Bool {
        nextSleepTimeMillisecondsGetCallsCount > 0
    }

    internal var nextSleepTimeMillisecondsGetCallsCount = 0
    /// `true` if the setter called on the property at least once.
    internal var nextSleepTimeMillisecondsSetCalled: Bool {
        nextSleepTimeMillisecondsSetCallsCount > 0
    }

    internal var nextSleepTimeMillisecondsSetCallsCount = 0
    /// The mocked property with a getter and setter.
    internal var nextSleepTimeMilliseconds: Milliseconds? {
        get {
            mockCalled = true
            nextSleepTimeMillisecondsGetCallsCount += 1
            return underlyingNextSleepTimeMilliseconds
        }
        set(value) {
            mockCalled = true
            nextSleepTimeMillisecondsSetCallsCount += 1
            underlyingNextSleepTimeMilliseconds = value
        }
    }
...
internal class SimpleTimerMock: SimpleTimer, TrackingMock {
    ...
    public func reset() {
        ...
        scheduleCallsCount = 0
        scheduleReceivedArguments = nil
        scheduleReceivedInvocations = []
    ...
    /// Number of times the function was called.
    internal private(set) var scheduleCallsCount = 0
    /// `true` if the function was ever called.
    internal var scheduleCalled: Bool {
        scheduleCallsCount > 0
    }

    /// The arguments from the *last* time the function was called.
    internal private(set) var scheduleReceivedArguments: (milliseconds: Milliseconds, block: () -> Void)?
    /// Arguments from *all* of the times that the function was called.
    internal private(set) var scheduleReceivedInvocations: [(milliseconds: Milliseconds, block: () -> Void)] = []
    /**
     Set closure to get called when function gets called. Great way to test logic or return a value for the function.
     */
    internal var scheduleClosure: ((Milliseconds, () -> Void) -> Void)?

    /// Mocked function for `schedule(milliseconds: Milliseconds, block: @escaping () -> Void)`. Your opportunity to return a mocked value and check result of mock in test code.
    internal func schedule(milliseconds: Milliseconds, block: @escaping () -> Void) {
        mockCalled = true
        scheduleCallsCount += 1
        scheduleReceivedArguments = (milliseconds: milliseconds, block: block)
        scheduleReceivedInvocations.append((milliseconds: milliseconds, block: block))
        scheduleClosure?(milliseconds, block)
    }
    ...
    /// The arguments from the *last* time the function was called.
    internal private(set) var scheduleIfNotAleadyReceivedArguments: (milliseconds: Milliseconds, block: () -> Void)?
    /// Arguments from *all* of the times that the function was called.
    internal private(set) var scheduleIfNotAleadyReceivedInvocations: [(milliseconds: Milliseconds,
                                                                        block: () -> Void)] =
        []
    /// Value to return from the mocked function.
     ...
     */
    internal var scheduleIfNotAleadyClosure: ((Milliseconds, () -> Void) -> Bool)?

    /// Mocked function for `scheduleIfNotAleady(milliseconds: Milliseconds, block: @escaping () -> Void)`. Your opportunity to return a mocked value and check result of mock in test code.
    internal func scheduleIfNotAleady(milliseconds: Milliseconds, block: @escaping () -> Void) -> Bool {
        mockCalled = true
        scheduleIfNotAleadyCallsCount += 1
        scheduleIfNotAleadyReceivedArguments = (milliseconds: milliseconds, block: block)
        scheduleIfNotAleadyReceivedInvocations.append((milliseconds: milliseconds, block: block))
        return scheduleIfNotAleadyClosure.map { $0(milliseconds, block) } ?? scheduleIfNotAleadyReturnValue
    }
