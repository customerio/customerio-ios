...
internal class IdentifyRepositoryMock: IdentifyRepository {
    ...
    /// The arguments from the *last* time the function was called. 
    internal private(set) var trackEventReceivedArguments: (name: String, data: RequestBody, jsonEncoder: JSONEncoder?, onComplete: (Result<Void, CustomerIOError>) -> Void)?
    /// Arguments from *all* of the times that the function was called. 
    internal private(set) var trackEventReceivedInvocations: [(name: String, data: RequestBody, jsonEncoder: JSONEncoder?, onComplete: (Result<Void, CustomerIOError>) -> Void)] = []
    /** 
     Set closure to get called when function gets called. Great way to test logic or return a value for the function. 
     */
    internal var trackEventClosure: ((String, RequestBody, JSONEncoder?, (Result<Void, CustomerIOError>) -> Void) -> Void)?
    ...
    internal func trackEvent<RequestBody: Encodable>(name: String, data: RequestBody, jsonEncoder: JSONEncoder?, onComplete: @escaping (Result<Void, CustomerIOError>) -> Void) {
        self.mockCalled = true
        trackEventCallsCount += 1
        trackEventReceivedArguments = (name: name, data: data, jsonEncoder: jsonEncoder, onComplete: onComplete)
        trackEventReceivedInvocations.append((name: name, data: data, jsonEncoder: jsonEncoder, onComplete: onComplete))
        trackEventClosure?(name, data, jsonEncoder, onComplete)
    }
