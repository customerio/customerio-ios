...
/**
 Class to easily create a mocked version of the `HttpErrorUtil` class.
 This class is equipped with functions and properties ready for you to mock!

 Note: This file is automatically generated. This means the mocks should always be up-to-date and has a consistent API.
 See the SDK documentation to learn the basics behind using the mock classes in the SDK.
 */
internal class HttpErrorUtilMock: HttpErrorUtil {
    /// If *any* interactions done on mock. `true` if any method or property getter/setter called.
    ...
    /// Number of times the function was called.
    internal private(set) var isIgnorableCallsCount = 0
    /// `true` if the function was ever called.
    internal var isIgnorableCalled: Bool {
        isIgnorableCallsCount > 0
    }

    /// The arguments from the *last* time the function was called.
    internal private(set) var isIgnorableReceivedArguments: (Error)?
    /// Arguments from *all* of the times that the function was called.
    internal private(set) var isIgnorableReceivedInvocations: [Error] = []
    /// Value to return from the mocked function.
    internal var isIgnorableReturnValue: Bool!
    /**
     Set closure to get called when function gets called. Great way to test logic or return a value for the function.
     The closure has first priority to return a value for the mocked function. If the closure returns `nil`,
     then the mock will attempt to return the value for `isIgnorableReturnValue`
     */
    internal var isIgnorableClosure: ((Error) -> Bool)?

    /// Mocked function for `isIgnorable(_ error: Error)`. Your opportunity to return a mocked value and check result of mock in test code.
    internal func isIgnorable(_ error: Error) -> Bool {
        mockCalled = true
        isIgnorableCallsCount += 1
        isIgnorableReceivedArguments = error
        isIgnorableReceivedInvocations.append(error)
        return isIgnorableClosure.map { $0(error) } ?? isIgnorableReturnValue
    }

    // MARK: - isHttpError

    /// Number of times the function was called.
    internal private(set) var isHttpErrorCallsCount = 0
    /// `true` if the function was ever called.
    internal var isHttpErrorCalled: Bool {
        isHttpErrorCallsCount > 0
    }

    /// The arguments from the *last* time the function was called.
    internal private(set) var isHttpErrorReceivedArguments: (Error)?
    /// Arguments from *all* of the times that the function was called.
    internal private(set) var isHttpErrorReceivedInvocations: [Error] = []
    /// Value to return from the mocked function.
    internal var isHttpErrorReturnValue: HttpRequestError?
    /**
     Set closure to get called when function gets called. Great way to test logic or return a value for the function.
     The closure has first priority to return a value for the mocked function. If the closure returns `nil`,
     then the mock will attempt to return the value for `isHttpErrorReturnValue`
     */
    internal var isHttpErrorClosure: ((Error) -> HttpRequestError?)?

    /// Mocked function for `isHttpError(_ error: Error)`. Your opportunity to return a mocked value and check result of mock in test code.
    internal func isHttpError(_ error: Error) -> HttpRequestError? {
        mockCalled = true
        isHttpErrorCallsCount += 1
        isHttpErrorReceivedArguments = error
        isHttpErrorReceivedInvocations.append(error)
        return isHttpErrorClosure.map { $0(error) } ?? isHttpErrorReturnValue
    }
}
