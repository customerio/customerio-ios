...
extension SdkCredentials {
  static let siteIdLens = Lens<SdkCredentials, String>(
    get: { $0.siteId },
    set: { siteId, existing in
       SdkCredentials(siteId: siteId, apiKey: existing.apiKey, region: existing.region)
    }
  )
  static let apiKeyLens = Lens<SdkCredentials, String>(
    get: { $0.apiKey },
    set: { apiKey, existing in
       SdkCredentials(siteId: existing.siteId, apiKey: apiKey, region: existing.region)
    }
  )
  static let regionLens = Lens<SdkCredentials, Region>(
    get: { $0.region },
    set: { region, existing in
       SdkCredentials(siteId: existing.siteId, apiKey: existing.apiKey, region: region)
    }
  )

  // Convenient set functions to edit a property of the immutable object
  func siteIdSet(_ siteId: String) -> SdkCredentials {
    SdkCredentials(siteId: siteId, apiKey: self.apiKey, region: self.region)
  }
  func apiKeySet(_ apiKey: String) -> SdkCredentials {
    SdkCredentials(siteId: self.siteId, apiKey: apiKey, region: self.region)
  }
  func regionSet(_ region: Region) -> SdkCredentials {
    SdkCredentials(siteId: self.siteId, apiKey: self.apiKey, region: region)
  }
}
